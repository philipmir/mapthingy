/**
 * Global Machine Monitor - Modern Design
 * 
...
// Create modern custom icons for different machine statuses
const createCustomIcon = (status, systemType, machineId, recentlyChanged) => {
  let color;
  let shape = 'circle'; // Default shape
  let size = 20; // Medium size for modern look
  
  // Set color based on status - vibrant modern colors
  switch (status) {
    ...
    case 'error': // Legacy support - map to red
      ...
      break;
    default:
      color = '#95a5a6'; // Default to grey
  }
  
  // Set shape based on system type - sleek modern shapes
  ...
  // Determine if this machine should pulse - smooth for modern
  const shouldPulse = recentlyChanged;
  const pulseClass = shouldPulse ? 'pulse-red' : '';
  const pulseColor = shouldPulse ? 'rgba(231, 76, 60, 0.8)' : 'rgba(255, 255, 255, 0.9)';
  const pulseAnimation = shouldPulse ? 'pulse-individual' : '';
  
  // Set border radius based on shape - rounded modern edges
  let borderRadius = '50%'; // Default circle
  if (shape === 'square') {
    borderRadius = '8px'; // Rounded corners for modern
  } else if (shape === 'diamond') {
    borderRadius = '4px'; // Slightly rounded diamond
  }
  ...
  return L.divIcon({
    ...
    html: `<div style="
      ...
      border: ${borderWidth} solid rgba(255, 255, 255, 0.9);
...
function AppModern() {
  ...
  if (CurrentStyleComponent && currentStyle !== 'modern') {
